...
The first problem can be found [here](http://codeforces.com/problemset/problem/652/D?locale=en).

*We are given $$n$$ segments: $$\langle l_1, r_1 \rangle, \langle l_2, r_2 \rangle, \ldots, \langle l_n, r_n \rangle$$ on a line. There are no coinciding endpoints among the segments.*
...
This problem can be solved by processing the segments in **decreasing order** of their left endpoints and maintaining a Fenwick tree that marks the right endpoints of the processed segments.

More precisely, once we have processed a segment $$\langle l_j, r_j \rangle$$, we add $$1$$ to position $$r_i$$ in the Fenwick tree. 

Now, assume that the current segment is $$\langle l_i, r_i \rangle$$. As we are processing segments in decreasing order of their left endpoint, we have already processed all the segments $$j$$ such that $$l_i < l_j$$, i.e., they start to the right of the current segment. 

How many of them are contained in $$\langle l_i, r_i \rangle$$? 

The answer is clearly the number of segments that end before $$r_i$$. This number can be computed with a query `sum`$$(r_i)$$ in $$O(\log n)$$ time.

<br>
##### Update the Array
This a [problem](http://www.spoj.com/problems/UPDATEIT/) from SPOJ.
...
*We are given an array $$A[1,n]$$, initially all the entries are set to $$0$$ and we would like to support two operations:*
    ...
    - `range_uptade(l, r, v)` *updates the entries in $$A[i..j]$$ by adding $$v$$.*
...
A Rust implementation of this solution is the following. We use the Fenwick tree of the previous section.
...
The range update of the previous problem is paired with the `access(i)` operation. This is easier than the problem we are going to solve in this section.
Here we want to support `range_update(l, r, v)` and `sum(i)` operations. Notice that `access(i)` is also immediately supported with `sum(i)` - `sum(i-1)`.  This pair of operations makes the problem harder than the previous one.

More formally, the problem is as follow.

*Given an array $$A[1,n]$$ of integers, we would like to support the following
operations.*
...
It's evident that we can solve a `range_update(l, r, v)` with $$j-i+1$$ `add` queries in $$\Theta((j-i+1)\log n)$$ time. However, our goal is to achieve a time complexity of $$\Theta(\log n)$$.
...
Let's consider a `range_update(l, r, v)` operation on an brand new Fenwick tree.
The correct results for a query `sum(i)` after the update are the following.
...
When we perform a `range_update(l, r, v)`, we add $$-v(l-1)$$ to position $$i$$ and $$v\cdot r$$ to position $$r+1$$ in $$FT_2$$.
...
These notes are for the [*"Competitive Programming and Contests"*](/rossano/competitive/) 
course at Università di Pisa.